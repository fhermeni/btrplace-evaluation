...
cnt = cnt[cnt$scenario == 2, c("duration","continuous")]
m = max(cnt$duration)
cnt$duration[is.na(cnt$duration)] <- 4000
cat("Maximum duration for discrete/sf ", max(cnt$duration[cnt$continuous == 0]), "\n")
conti = cnt[cnt$continuous == 1, ]
dist = cnt[cnt$continuous == 0, ]
oh = (1 - mean(dist$duration) / mean(conti$duration)) * 100
cat("Computational overhead for continuous/sf: " , oh, "%\n")
colors <- c(grey(0.8), grey(0))
w=5
plot(ecdf(conti$duration), yaxt='n', xlim=c(5,m), col=grey(0), log="x", lty=1, do.points=FALSE, lwd=w, xlab="Duration (sec.)", ylab="Solved instances", main="", verticals = TRUE, panel.first=grid())
legend(20,0.3, c("discrete","continuous"), col=colors, cex=1, lty=1, lwd=w, border = FALSE, bty="n")
