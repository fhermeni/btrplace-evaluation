...
cnt = cnt[cnt$scenario == "sf", c("duration","continuous")]
m = max(cnt$duration)
...
w=5
plot(ecdf(conti$duration), yaxt='n', xlim=c(5,m), col=grey(0), lty=1, do.points=FALSE, lwd=w, xlab="Duration (sec.)", ylab="Solved instances", main="", verticals = TRUE, panel.first=grid())
legend(10,0.3, c("discrete","continuous"), col=colors, cex=1, lty=1, lwd=w, border = FALSE, bty="n")
